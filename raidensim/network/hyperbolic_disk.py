...
class HyperbolicDisk:
    ...
    def __init__(self, num_rings: int, radius: float):
        self.num_rings = num_rings
        self.radius = radius
        self.cosh_radius = math.cosh(radius)
        self.node_to_coord = {}
        self.coord_to_node = {}

    @property
    def num_slots(self):
        return 2**(self.num_rings + 1) - 1
    ...
    def coord_to_polar(self, coord: DiskCoord) -> PolarCoord:
        r = coord[0] / self.num_rings * self.radius
        theta = coord[1] * 2 * math.pi / 2**coord[0]
    ...
    def outer_coord_partners(self, coord: DiskCoord) -> Iterator[np.array]:
        return self.coord_partners(coord, (coord[0] + 1, self.num_rings))
    ...
    def outer_partner_coords(self, coord: DiskCoord) -> Iterator[np.array]:
        return self.partner_coords(coord, (coord[0] + 1, self.num_rings))
    ...
    def partner_coords(self, coord: DiskCoord, rings: IntRange):
        ...
        for r in range(rings[0], rings[1] + 1):
            ...
            num_ring_slots = 2**r
            # Check nodes on each ring by stepping outward from the closest node.
            for di in range(0, num_ring_slots // 2):
                ...
                num_new_nodes = 0
                if self.coord_distance(coord, [r, i1]) <= self.cosh_radius:
                    num_new_nodes += 1
                    yield np.array([r, i1])
                if di == 0:
                    continue
                if self.coord_distance(coord, [r, i2]) <= self.cosh_radius:
                    num_new_nodes += 1
                    yield np.array([r, i2])
    ...
    def polar_distance(a: PolarCoord, b: PolarCoord):
        a_r, a_theta = a
        b_r, b_theta = b
        if math.isclose(a_r, b_r) and math.isclose(a_theta, b_theta):
            return 0
        else:
            return math.cosh(a_r) * math.cosh(b_r) - \
